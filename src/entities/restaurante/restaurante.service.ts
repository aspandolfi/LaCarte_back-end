...
export class RestauranteService implements IServiceBase<Restaurante> {
    ...
    if (errors.length == 0) {
      ...
      if (result === undefined) {
        this.response.mensagens.push(
          "Erro ao salvar restaurante no banco de dados."
        );
        return this.response;
      }
  ...
  async drop(id: number): Promise<Restaurante | ResponseData> {
    let query = await this.restauranteRepository.findOneById(id);
    ...
    let result = await this.restauranteRepository.remove(query);
